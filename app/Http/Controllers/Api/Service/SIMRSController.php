...
class SIMRSController extends Controller
{
    ...
    public function getListAntrianFarmasi(Request $request)
    {
        ...
        select 
        aa.noantri as aanoantri,aa.jenis as aajenis,apr.jenis as jenisapr,apr.noantrian as noantrianapr,apr.noantrianpoli,ps.namapasien,st.statuspengerjaan as statusorder,aa.tglresep
        from antrianapotik_t as aa 
        join pasiendaftar_t as pd on pd.noregistrasi = aa.noregistrasi 
        join antrianpasienregistrasi_t as apr on apr.norec = pd.antrianpasienregistrasifk
        join pasien_m as ps on ps.id = pd.nocmfk
        left join statuspengerjaan_m as st on st.id = aa.status 
        WHERE aa.tglresep between '$tglAwal' and '$tglAkhir' and aa.status != 6
